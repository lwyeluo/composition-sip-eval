...
# $1 is the dataset path
# $2 is the combination path
# $3 is the output path
if [ "$#" -lt 4 ]; then
	echo "Usage: bash generator.sh objective dataset-path combination-path output-path [constraints]"
	exit 1
fi
...
binary_path=$4
#binary_path=/home/sip/eval/binaries-manifest
CONSTRAINTS='NONE'
if [[ "$#" -eq 5  ]];
then 
	CONSTRAINTS=$5
...
do
	...
	do
		...
		do
			...
			if [ "$CONSTRAINTS" != "NONE" ];
			then
				echo 'CONSTRAINTS is not NONE'
				constraint_file=${CONSTRAINTS}/${filename}/${coverage_name}/${combination_file}
				if [ -f "$constraint_file" ]; then
					constraints_args=$(<${constraint_file})
				else
					echo "did not fid the constraint file ${constraint_file}"
					exit 1
				fi
			...
			do
				...
				echo 'Computing resilience...'
				python3 compute-resilience.py ${output_dir} ${output_dir}
				#echo 'Obfuscating...'
