...
def dump_coverage_table():
    ...
    table_headers = ["Program", "Blocks", "LB", "OHB%", "SROHB%", "OHI%", "SROHI%", "OHI", "SROHI", "SkippedI",
                     "SROHDDI"]
    table_data = []
    for key in programs:
...
def dump_latex_table_for_paper(with_dg=True):
    ...
    oh_instr_coverage_headers = ["Program", "Instructions", "SROHI%", "OHI%", "OH", "SROH", "SROH DDI", "LI+ARI+GRI",
                                 "CDF", "IAI", "NHI"]
    ...
    for key in programs:
        ...
        unprot_instr = (unprotected_loop_instr[key] + unprotected_loop_variant_instr[key] +
                        unprotected_argument_reachable_instr[key] + unprotected_global_reachable_instr[
                            key] + unprot_cfd)
        ...
        oh_instr_coverage_data.append([program, instrs, sroh_prot_instr_cov, oh_prot_instr_cov,
                                       oh_prot_instr, sroh_prot_instr, sroh_prot_ddi_instr, li_ari_gri, unprot_cfd,
                                       unprotected_instr_with_no_dg, non_hashable_instr])

        oh_block_coverage_data.append([program, sensitive_blcks, sroh_prot_block_cov, oh_prot_block_cov, loop_blocks,
                                       oh_prot_instr, sroh_prot_instr, unprotected_blocks_with_no_dg,
                                       non_hashable_block])
...
def dump_latex_table():
    ...
    oh_block_coverage_headers = ["program", "sensitive blocks", "oh protected blocks", "short oh protected blocks",
                                 "oh protected block %",
                                 "short oh protected block %", "non-hashable blocks", "unprotected loop blocks",
                                 "unprotected data dep blocks"]
    oh_instruction_coverage_headers = ["program", "LLVM instructions", "oh protected instr", "short oh protected instr",
                                       "non-hashable instr", "unprotected loop instr",
                                       "unprotected arg. reachable instr", "unprotected data dep. instr",
                                       "unprotected instr", "unprotected instr in filtered functions",
                                       "unprotected instr in functions with no dg", "oh processed instr"]
    ...
    for key in programs:
        ...
        data.append([program, instrs, input_indep, input_indep_cov, input_dep_cov, data_indep, arg_deps, loop_deps,
                     data_indep_cov])
        ...
        oh_instruction_data.append([program, instrs, oh_prot_instr, short_oh_prot_instr, non_hashable_instr, loop_instr,
                                    arg_reach_instr, data_dep_inst, unprot_instr, unprot_filtered_instr,
                                    unprot_no_dg_instr, oh_processed_instr])
...
def parse_stats(bitcode_name, dir_name):
    ...
    input_indep_instr[bitcode_name] = stats[INPUT_DEP_STATS_KEY][module_name_key][INPUT_INDEP_COVERAGE_KEY][
        INPUT_INDEP_INSTR_KEY]
    ...
    data_indep_instr[bitcode_name] = stats[INPUT_DEP_STATS_KEY][module_name_key][DATA_INDEP_COVERAGE_SECTION_KEY][
        DATA_INDEP_INSTR_KEY]
    argument_dep_data_indep_instr[bitcode_name] = \
    stats[INPUT_DEP_STATS_KEY][module_name_key][DATA_INDEP_COVERAGE_SECTION_KEY][ARGUMENT_DEP_DATA_INDEP_INSTR_KEY]
    argument_or_data_dep_loop_instr[bitcode_name] = \
    stats[INPUT_DEP_STATS_KEY][module_name_key][DATA_INDEP_COVERAGE_SECTION_KEY][
        DATA_INDEP_INSTR_IN_DATA_OR_ARGUMENT_DEP_LOOP_KEY]
    data_indep_cov = stats[INPUT_DEP_STATS_KEY][module_name_key][DATA_INDEP_COVERAGE_SECTION_KEY][
        DATA_INDEP_COVERAGE_KEY]
