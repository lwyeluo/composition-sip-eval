...
def grab_results(result_directory, binary_name):
    ...
    before_clone_re, after_clone_re = load_result_file(binary_name, result_directory, "clone.stats", BEFORE_CLONE_KEY,
                                                       AFTER_CLONE_KEY, CLONE_COVERAGE_KEY)
    print "clone result"
    pprint(before_clone_re)
    pprint(after_clone_re)
    before_extract_re, after_extract_re = load_result_file(binary_name, result_directory, "extract.stats",
                                                           BEFORE_EXTRACT_KEY, AFTER_EXTRACT_KEY, EXTRACT_COVERAGE_KEY)
    print "extract result"
    pprint(before_extract_re)
    pprint(after_extract_re)
    return {"before_clone": before_clone_re, "after_clone": after_clone_re, "before_extract": before_extract_re,
            "after_extract": after_extract_re}
